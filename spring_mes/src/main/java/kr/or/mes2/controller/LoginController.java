...
public class LoginController {
	...
	//  리셋코드 로그인
		...
		if (tokenUser != null) {
		    ...
		    session.setAttribute("loginUserId", tokenUser.getUserId());
		    session.setAttribute("isAdmin", "ADMIN".equals(tokenUser.getRole()));

		    // ✅ 새로 추가: resetToken 세션에 임시 저장
		    session.setAttribute("resetLoginId", loginId);
		    session.setAttribute("resetToken", password);
